...
class BowDSSMInference:
    ...
    def _load_hub_encoder(self) -> None:
        ...
        self.hub_to_vec: Dict[str, np.ndarray] = {
            hub: np.array(vec, dtype=np.float32)
            for hub, vec in hub_dict.items()
        }
        ...
        print(f"Hub encoder: {len(self.hub_to_vec)} hubs, dim={self.hub_dim}")
